...
// Interface para o componente original (compatibilidade)
...
// Interface para o novo componente (assessment)
...
// Componente original (default export para compatibilidade)
export default function CustomPhoneInput({
    ...
    if (value) {
      ...
      if (cleanValue.startsWith("55")) {
        ...
        if (numberWithoutCountry.length <= 2) {
          ...
          setPhoneNumber(
            `${numberWithoutCountry.slice(0, 2)} ${numberWithoutCountry.slice(2, 7)}-${numberWithoutCountry.slice(7, 11)}`
          )
        }
      } else {
        // Format the number for display
        if (cleanValue.length <= 2) {
          ...
          setPhoneNumber(
            `${cleanValue.slice(0, 2)} ${cleanValue.slice(2, 7)}-${cleanValue.slice(7, 11)}`
          )
        }
      }
    }
  ...
  return (
    <div>
      <div
        className={`flex h-14 w-full rounded-xl border-2 ${
          error ? "border-red-500 bg-red-50" : "border-gray-200 bg-gray-50 hover:border-gray-300"
        } focus-within:border-[#a3ff3c] focus-within:bg-white focus-within:ring-4 focus-within:ring-[#a3ff3c]/10 px-4 py-3 transition-all`}
      >
        <input
          type="tel"
          value={phoneNumber}
...
// Novo componente simplificado para assessment (named export)
